...
# --- patterns/4-advanced_routing/3-inspection_after_filtering/terraform/cloudwan_policy.tf ---
...
data "aws_networkmanager_core_network_policy_document" "policy_document" {
  ...
  attachment_routing_policy_rules {
    ...
    action {
      associate_routing_policies = ["allowCidrRange"]
    }
  }

  routing_policies {
    routing_policy_name        = "allowCidrRange"
    routing_policy_description = "Allowing 10.0.0.0/8 CIDR range"
