...
**Regenerate from a directory with no `.terraform/` in it.** terraform-docs reads the *resolved* provider versions out of `.terraform/` when it is present, and the *constraints* declared in the code when it is not. CI generates from a clean checkout, so the committed README must carry the constraints (`>= 6.34.0`), not whatever a local `terraform init` happened to resolve (`6.58.0`). Regenerating in an initialized directory therefore produces a README that CI rejects — and because providers are floor-pinned and [`.terraform.lock.hcl` is not committed](CONVENTIONS.md#3-version-pins), a resolved version baked into a README also goes stale the next time the provider is released. `terraform init` recreates `.terraform/` whenever you need it back:

```bash
rm -rf infra/<pattern>/terraform/.terraform
terraform-docs --config .config/.terraform-docs.yaml infra/<pattern>/terraform
```

Note that the local `terraform_docs` pre-commit hook has the same behaviour: run it in an initialized directory and it will rewrite the README with resolved versions and report `files were modified by this hook`. That is the hook disagreeing with CI about the tree it is looking at, not a genuine drift.
