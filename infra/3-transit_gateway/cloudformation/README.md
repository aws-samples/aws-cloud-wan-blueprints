# Infra pattern 3 — Transit Gateway (AWS CloudFormation)
...
| `workloads.yaml` | Transit Gateway, route tables, Cloud WAN peering, route-table attachments, spoke VPCs, instances | Once per Region |
...
```bash
python3 tools/sync_cfn_policy.py infra/3-transit_gateway
```
...
Everything: core network first, then the Transit Gateway and workloads in both Regions:
