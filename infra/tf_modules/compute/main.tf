...
# --- infra/tf_modules/compute/main.tf ---
...
resource "aws_vpc_security_group_ingress_rule" "allowing_ingress_icmp" {
  ...
  description = "Allow ICMP from anywhere, so cross-segment and cross-Region ping tests work"
  from_port   = -1
  to_port     = -1
  ip_protocol = "icmp"
  cidr_ipv4   = "0.0.0.0/0"
...
resource "aws_vpc_security_group_ingress_rule" "allowing_ingress_eic" {
  ...
  description                  = "Allow SSH only from the EC2 Instance Connect endpoint security group"
  from_port                    = 22
  to_port                      = 22
  ip_protocol                  = "tcp"
...
resource "aws_vpc_security_group_egress_rule" "allowing_egress_any" {
  ...
  description = "Allow all egress, so the instance can reach other segments and the internet via inspection"
  ip_protocol = "-1"
  cidr_ipv4   = "0.0.0.0/0"
}
...
resource "aws_vpc_security_group_egress_rule" "allowing_egress_ec2_instances" {
  ...
  description                  = "Allow the EC2 Instance Connect endpoint to reach the instances over SSH"
  from_port                    = 22
  to_port                      = 22
  ip_protocol                  = "tcp"
